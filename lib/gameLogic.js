...
function pickFormation(room, clientId, formationName){
...
function assignSlot(room, clientId, cat, idx, playerId){
  const e = findManagerByClient(room, clientId); if(!e) return {error:"You haven't joined this room."};
  const m = e[1];
  ...
  m.slots[cat+idx] = playerId;
  return {};
}
function clearSlot(room, clientId, cat, idx){
  const e = findManagerByClient(room, clientId); if(!e) return {error:"You haven't joined this room."};
  delete e[1].slots[cat+idx];
  return {};
}
...
module.exports = {
  ...
  pickFormation, assignSlot, clearSlot, autofillSlots, lockSquad, autoLockManager,
