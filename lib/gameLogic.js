const { PLAYERS, FORMATIONS, BUDGET, buildOrder, nextIncrement, AUCTION_POS_RANK, LEAGUES } = require("./gameData");

const LOT_SECONDS = 12;
const REBID_SECONDS = 23;
const SQUAD_SECONDS = 120;
const MAX_SQUAD = 11;
const ALL_LEAGUE_KEYS = LEAGUES.map(l=>l.key);
...
function createRoom(code, hostClientId){
  return {
    ...
    leagues: ALL_LEAGUE_KEYS.slice()
  };
}
function setLeagues(room, clientId, leagueKeys){
  if(room.hostClientId!==clientId) return {error:"Only the host can change this."};
  if(room.phase!=="lobby") return {error:"Leagues can only be changed before the auction starts."};
  const clean = Array.isArray(leagueKeys) ? leagueKeys.filter(k=>ALL_LEAGUE_KEYS.includes(k)) : [];
  if(clean.length===0) return {error:"Pick at least one league."};
  room.leagues = [...new Set(clean)];
  return {};
}
...
function startAuction(room, clientId){
  ...
  const leagues = (room.leagues && room.leagues.length) ? room.leagues : ALL_LEAGUE_KEYS;
  const eligibleIds = PLAYERS.filter(p=>leagues.includes(p.league)).map(p=>p.id);
  if(eligibleIds.length<11) return {error:"Not enough players in the selected leagues — pick at least one more league."};
  room.order = buildOrder(eligibleIds);
  room.cur = 0;
...
function redactRoomFor(room, clientId){
  ...
  return {
    ...
    leagues: room.leagues || ALL_LEAGUE_KEYS
  };
}
...
module.exports = {
  createRoom, findManagerByClient, joinAsManager, startAuction, setLeagues, placeBid, passLot, finalizeLot, skipToNextCategory,
  ...
  computeRecords, redactRoomFor, mergeHof, LOT_SECONDS, REBID_SECONDS, SQUAD_SECONDS, MAX_SQUAD, ALL_LEAGUE_KEYS
};
